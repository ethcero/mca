...
import es.ethcero.mca.practica2.ui.command.InsuranceCommand;
...
public class ClientController {
    ...
    ResponseEntity newClient(@RequestBody NewClientCommand newClientCommand) {
        clientProcess.addClient(newClientCommand.getName());
        return ResponseEntity.ok().build();
    }

    @PostMapping("/{clientId}/insurance")
    ResponseEntity newInsurance(@PathVariable long clientId, @RequestBody InsuranceCommand insuranceCommand) {
        clientProcess.addInsurance(clientId, insuranceCommand.getCoverages());
        return ResponseEntity.ok().build();
    }

    @PostMapping("/{clientId}/issue")
    ResponseEntity newIssue(@PathVariable long clientId, @RequestBody NewIssueCommand newIssueCommand) {
        clientProcess.addIssue(clientId, newIssueCommand.getAmount(), newIssueCommand.getCoverage());
        return ResponseEntity.ok().build();
    }
